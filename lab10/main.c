...
int* get_dst(int rank, int numProcs, int leader) {
	...
	*/
	...
	for (int i = 0; i < num_neigh; ++i)
	{
		if (neigh[i] != v[rank])
		{
			MPI_Recv(vRecv, numProcs, MPI_INT, neigh[i], 0, MPI_COMM_WORLD, NULL);
	...
	*/
	if(rank != leader)
	{
		...
		MPI_Recv(vRecv, numProcs, MPI_INT, v[rank], 0, MPI_COMM_WORLD, NULL);
	}
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (int i = 0; i < num_neigh; ++i)
		{
			...
			MPI_Recv(&neigh_leader, 1, MPI_INT, neigh[i], 0, MPI_COMM_WORLD, NULL);
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (int i = 0; i < num_neigh; ++i)
		{
			...
			MPI_Recv(&recv_val, 1, MPI_DOUBLE, neigh[i], 0, MPI_COMM_WORLD, NULL);
			val = (val + recv_val) / 2;
		}
	}
