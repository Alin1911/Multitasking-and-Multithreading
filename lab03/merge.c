...
void *thread_function(void *arg)
{
	...
	int start_index = thread_id * (double) N / P;
	int end_index = MIN(N , (thread_id + 1) * (double)N/P);
	int *aux;

	for (int width = 1; width < N; width = 2 * width) {
		int start_local = (start_index / (2 * width)) * (2 * width);
		int end_local = MIN(N, (end_index / (2 * width)) * (2 * width));
